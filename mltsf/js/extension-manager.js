...
window.ExtensionAPI = {
     ...
     */
    registerCommand(cmdName, definition) {
        if (!this._manager) return;
        const name = cmdName.startsWith('/') ? cmdName : '/' + cmdName;
        this._manager._registerCommandFromExtension(name, definition);
    },
...
let extensionManager = {
    ...
    async loadExtensionFromUrl(url) {
        ...
        if (!url.startsWith('https://raw.githubusercontent.com/') && !url.startsWith('https://github.com/')) {
            terminal.println('警告: 该链接不是 GitHub 链接，仍将尝试加载', 'warning-line');
        }
    ...
    async _installExtensions(repoPath, extList) {
        ...
        for (const ext of extList) {
            ...
            try {
                ...
                this._saveState();
                terminal.println(`✅ '${ext.name}' 安装成功 (${commandsToRegister.length} 个命令)`, 'success-line');
                successCount++;
            } catch (err) {
    ...
    _loadState() {
        try {
            ...
            if (saved) {
                ...
                if (data.loadedExtensions) this.loadedExtensions = data.loadedExtensions;
            }
        } catch (e) {

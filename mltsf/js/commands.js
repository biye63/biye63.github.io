...
async function sendToAI(question) {
    ...
    try {
        ...
        terminal.println('');
        // 判断是否为网络错误（CORS / 跨域）
        if (error.message === 'Failed to fetch' || error.message.includes('NetworkError') || error.message.includes('Load failed') || error.message.includes('network')) {
            terminal.println('⚠ 这可能是跨域问题：请使用启动脚本打开页面而非直接双击 HTML！', 'warning-line');

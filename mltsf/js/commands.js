 ...
 * @param {boolean} isPassword - 是否为密码输入（隐藏输入内容）
 */
function createPromptInput(promptText, defaultValue = '', isPassword = false) {
    return new Promise((resolve) => {
        const lineContainer = document.createElement('div');
        lineContainer.style.cssText = 'display:flex;align-items:center;margin-top:5px;';
        
        // 使用 DOM API 创建元素，避免 innerHTML 注入问题
        const label = document.createElement('span');
        label.style.cssText = 'color:#00ff00;white-space:nowrap;';
        label.textContent = promptText;
        lineContainer.appendChild(label);
        
        const input = document.createElement('input');
        input.type = isPassword ? 'password' : 'text';
        // 使用 DOM API 设置 value，彻底避免 HTML 注入问题
        input.value = defaultValue;
        input.style.cssText = 'background:#000;color:#fff;border:1px solid #00ff00;border-radius:2px;outline:none;font-family:inherit;font-size:14px;flex:1;margin-left:4px;padding:2px 4px;';
...
async function aiCommand(args) {
    if (!args || args.length === 0) {
        ...
        terminal.println('');
        terminal.println('  /ai code          - 配置 AI 参数 (API Key、URL、模型)', 'info-line');
        terminal.println('  /ai 用户的问题    - 向 AI 提问，AI 可执行终端命令', 'info-line');
...
async function configureAI() {
    ...
    terminal.println('');
    terminal.println('现在可以使用 /ai 用户的问题 向 AI 提问', 'info-line');
}
...
async function sendToAI(question) {
    ...
    try {
        ...
        terminal.println(`请求地址: ${apiUrl}`, 'info-line');
        terminal.println(`模型: ${config.model || 'gpt-4'}`, 'info-line');
        terminal.println('');
        
        // 创建 AbortController 以实现超时控制（15秒无响应则中断）
        ...
        terminal.println('  1. API URL 是否正确（例如 https://api.openai.com/v1）', 'info-line');
        terminal.println('  2. API Key 是否有效', 'info-line');
        terminal.println('  3. 模型名称是否正确（例如 gpt-4、deepseek-chat）', 'info-line');
        terminal.println('  4. 网络连接是否正常', 'info-line');
        terminal.println('');
        // 判断具体错误类型并提供针对性建议
        if (error.name === 'AbortError') {
            terminal.println('⏱ AI 请求超时（15秒无响应），请检查网络或 API 服务状态', 'warning-line');
        } else if (error.message.includes('401')) {
            terminal.println('🔑 API Key 无效或未授权，请检查密钥是否正确', 'warning-line');
            terminal.println('   注意：某些模型需要特定 API Key，例如 DeepSeek 需使用 DeepSeek 的 Key', 'info-line');
        } else if (error.message.includes('403')) {
            terminal.println('🚫 API 拒绝访问，密钥可能无权限或已过期', 'warning-line');
        } else if (error.message.includes('429')) {
            terminal.println('📊 API 请求频率过高或额度不足，请稍后再试', 'warning-line');
        } else if (error.message.includes('5') && parseInt(error.message.match(/\b(5\d{2})\b/)?.[1]) >= 500) {
            terminal.println('🔧 API 服务端异常，请稍后重试', 'warning-line');
        } else if (error.message === 'Failed to fetch' || error.message.includes('NetworkError') || error.message.includes('Load failed') || error.message.includes('network') || error.message.includes('TypeError')) {
            terminal.println('⚠ 网络请求失败，这可能是跨域问题：请使用启动脚本打开页面而非直接双击 HTML！', 'warning-line');
            terminal.println('   运行 mltsf 文件夹中的 start.bat，然后访问 http://localhost:3000', 'warning-line');
        }
        terminal.println('');
        terminal.println('⚠ 安全提示：API Key 存储在浏览器 localStorage 中，请勿在公共设备上使用', 'warning-line');

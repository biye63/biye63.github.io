...
window.ExtensionAPI = (function() {
    ...
    function printHtml(html, className) {
        if (window.terminal) {
            window.terminal.printHtml(html, className);
        }
    }
    ...
    function _resolveUrl(url) {
        ...
        // 相对路径，拼接到CDN基URL
        if (_cdnBaseUrl) {
            var base = _cdnBaseUrl.endsWith('/') ? _cdnBaseUrl : _cdnBaseUrl + '/';
            return base + url;
        }
        return url;
    }
    ...
    function createUI(html, options) {
        ...
        container.className = 'extension-ui-panel';
        ...
        if (options.title) {
            ...
            header.className = 'extension-ui-header';
            header.innerHTML = '<span class="extension-ui-title">' + _escapeHtml(options.title) + '</span>';

            if (options.closable !== false) {
                var closeBtn = document.createElement('button');
                closeBtn.className = 'extension-ui-close';
        ...
        content.className = 'extension-ui-content';
    ...
    function showDialog(options) {
        return new Promise(function(resolve) {
            ...
            var html = '<div class="extension-dialog-overlay">' +
                '<div class="extension-dialog" style="' + (options.width ? 'max-width:' + options.width + '%' : '') + '">' +
                '<div class="extension-dialog-header">' + _escapeHtml(title) + '</div>' +
                '<div class="extension-dialog-body">' + message;

            if (type === 'prompt') {
                html += '<br><input type="text" class="extension-dialog-input" placeholder="' +
                    _escapeHtml(inputPlaceholder) + '" autofocus>';
            }

            html += '</div>' +
                '<div class="extension-dialog-footer">' +
                '<button class="extension-dialog-btn extension-dialog-btn-confirm">' + _escapeHtml(confirmText) + '</button>';

            if (type !== 'alert') {
                html += '<button class="extension-dialog-btn extension-dialog-btn-cancel">' + _escapeHtml(cancelText) + '</button>';
            }
            ...
            var confirmBtn = overlay.querySelector('.extension-dialog-btn-confirm');
            var cancelBtn = overlay.querySelector('.extension-dialog-btn-cancel');
            var input = overlay.querySelector('.extension-dialog-input');

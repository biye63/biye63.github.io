let terminal = {
    ...
    printWelcomeMessage() {
        ...
        this.println('║              MLTSF - 多语言终端模拟器 v2.23              ║', 'info-line');
    ...
    async processSystemCommand(command) {
        ...
        switch (cmd) {
            ...
            case '/lxe':
                ...
                break;
            case '/查看扩展包':
                await extensionManager.getAvailableExtensions();
                break;
            case '/ap':
                await extensionManager.loadExtension(args[0], args[1]);
                break;
            case '/rm':
                await extensionManager.unloadExtension(args[0]);
                break;
            case '/az':
                await extensionManager.loadExtensionFromUrl(args.join(' '));
                break;
            case '/ar':
                await extensionManager.autoRecognizeExtension(args.join(' '));
                break;
            case '/wr':
                writeFile(args);
                break;
            ...
            default:
                // 检查是否是扩展包注册的命令
                if (extensionManager.registeredCommands[cmd]) {
                    const extCmd = extensionManager.registeredCommands[cmd];
                    try {
